...
_SYSTEM_PROMPT = (
    "You are designing the response schema for an AI agent's document "
    "extraction pipeline. The agent has already classified this "
    "document and decided to process it. Your job is to propose the "
    "Pydantic shape the agent will receive so it can take the next "
    "action on the data.\n\n"
    "Think like the agent's user. They are not asking 'what is on "
    "this page?'. They are asking 'what can I do with what is on "
    "this page?'. The answer is whatever drives the next step in "
    "their workflow.\n\n"
    "Examples of how to reason:\n"
    "- A payroll register is uploaded because the agent will validate "
    "amounts, sum contributions, or route specific rows for approval. "
    "The agent needs the per-row data (identifiers, days, "
    "contribution amounts, totals), not the document title or page "
    "number.\n"
    "- An invoice is uploaded because the agent will match it to a "
    "purchase order and queue payment. The agent needs the vendor, "
    "line items, totals, and tax, not the bank's branding.\n"
    "- A government ID is uploaded because the agent will run KYC "
    "checks. The agent needs the holder's identity fields and expiry "
    "date, not the seal description.\n\n"
    "Skip extracting anything an agent cannot act on: document "
    "titles, headers like 'SISTEMA UNICO DE ...', page numbers, "
    "form section labels, layout decorations. If a value is purely "
    "metadata about the document rather than data inside the "
...
_USER_PROMPT = (
    "Look at this page. Propose the Pydantic schema the AI agent's "
    "pipeline should receive: only the fields the agent can act on. "
    "Skip document metadata, titles, and layout decorations. "
