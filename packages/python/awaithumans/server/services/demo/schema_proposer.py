...
_SYSTEM_PROMPT = (
    "You are designing the response schema for an AI agent's "
    "document extraction pipeline. The agent has already classified "
    "this document and decided to process it. It does not need to "
    "know what kind of document this is. It needs the data inside "
    "the document so it can take the next operational step.\n\n"
    "Reason about the page by distinguishing two kinds of content:\n"
    "- Operational content: the data the document was created to "
    "carry. It is the reason the document exists. Software will "
    "compute on it, validate it, match it against other records, "
    "route it for approval, or hand it to a downstream system. "
    "Repeated rows, line items, transaction records, amounts, "
    "identifiers tied to a real-world subject, dates that govern an "
    "obligation, or fields a human had to fill in are all "
    "operational content.\n"
    "- Metadata: signals that exist to tell a human reader what kind "
    "of document this is. Document titles, institutional banners, "
    "form section labels, decorative branding, version stamps, and "
    "page numbers fall here. The agent already knows the answer "
    "those signals would convey, so they have no operational value.\n\n"
    "Two heuristics:\n"
    "1. If a value is repeated many times on the page with different "
    "instances (rows of a table, items in a list), it is almost "
    "certainly operational. Capture it as `list[record]` with the "
    "row's columns as fields.\n"
    "2. If a value is a singleton at the very top of the page, "
    "rendered in a heading font, and would not change meaning if you "
    "stripped the rest of the document away, it is probably metadata "
    "and should be excluded.\n\n"
    "When in doubt, ask: would an agent that already knows what kind "
    "of document this is have any use for this field? If the answer "
    "is no, drop it.\n\n"
    "Rules:\n"
    ...
    "8. Field count follows the data: extract as many as the "
    "operational content warrants, no fewer, no more.\n\n"
...
_USER_PROMPT = (
    "Look at this page. Propose the schema an AI agent's pipeline "
    "should receive. Reason about which content is operational "
    "versus which content is metadata, and include only the "
    "operational fields. "
